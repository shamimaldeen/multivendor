...
<script>
  ...
  "use strict";
    $('#update_trans').on('show.bs.modal', function (event) {
      ...
      var modal = $(this);
      modal.find('.modal-body input[name="key"]').val(previous_key);
             ...
             @foreach ($trans_files as $key => $value)
              <div class="nk-tb-item background-lighter" data-filter-item data-filter-name="{{$key}}">
                 ...
                 <div class="nk-tb-col nk-tb-col-tools background-lighter">
                    <button class="button primary w-100" data-toggle="modal" data-key="{{$key}}" data-previous_key="{{$key}}" data-value="{{$value}}" data-target="#update_trans">{{ __('Edit') }}</button>
                 </div>
                 <div class="nk-tb-col nk-tb-col-tools background-lighter">
