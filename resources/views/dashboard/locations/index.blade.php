...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
            <a class="button text-white bg-theme-1 shadow-md mr-2" href="{{ url('dashboard/locations/create') }}">Add New Location</a>
            <div class="dropdown">
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                ...
                <tr>
                    ...
                    <th class="text-center whitespace-no-wrap">Region/District Name</th>
                ...
                @foreach($locations as $location)
                    @if($location->id!=1)
                        <tr class="intro-x">
                            <td>
                                <div class="text-gray-600 text-xs whitespace-no-wrap">{{ $location->name }}</div>
                            </td>
                            <td class="text-gray-600 text-xs whitespace-no-wrap">{{ $location->parent->name }}</td>
                            <td class="w-40">
                                <div class="flex items-center justify-center text-theme-9"> <i data-feather="check-square" class="w-4 h-4 mr-2"></i>@if($location->status) Active @else Inactive @endif </div>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{ url('dashboard/locations/edit/'.$location->id) }}"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
