...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-6">
            <!-- BEGIN: Form Layout -->
            <form action="{{url('dashboard/locations/store')}}" method="POST">
                @csrf
                <div class="intro-y box p-5">
                    <div>
                        <label>Location Name</label>
                        <input type="text" class="input w-full border mt-2 @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}">
                        @error('name')
