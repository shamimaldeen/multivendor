                ...
                <form method="get" action="{{ settings('business.enabled') ? url("dashboard/invoice/$plan->slug") : '' }}" role="form">
                    <div class="row d-flex align-items-stretch mb-4 mb-3 bdrs-20 background-lighter p-md-3 p-2">
                        @foreach ($plan->price as $key => $value)
                            @if ($value !== NULL)
                            <label class="col-md-4 col-6 mb-4 mt-4 pricing-select">
                            <input type="radio" name="payment_plan" value="{{$key}}" class="custom-control-input" required="required" id="{{$key}}_price" data-price="{{ number_format($value) }}">
                                <div class="pricing-select-inner">
                                  <div class="mt-3 text-center mb-1">
                                    ...
                                    <label for="{{$key}}_price"><i class="tio thumbs_up"></i></label>
                                </div>
                            </label>
                            @endif
                        @endforeach
                ...
                @if (env('PAYPAL_STATUS') || env('STRIPE_STATUS') || env('RAZOR_STATUS') || env('BANK_STATUS') || env('MIDTRANS_STATUS') || env('PAYSTACK_STATUS'))
                  <div class="row">
                      ...
                      <div class="card p-4 card-shadow border-0">
                        ...
                        <div class="row d-flex align-items-stretch mb-4">
                           @foreach ($gateway as $key => $item)
                               ...
                               <div class="col-md-4 col-6 other-pricing">
                               ...
                               @endif
                           @endforeach
                               <div class="col-md-4 col-6 hide free-pricing">
                                  <label class="big-radio">
                                     <input type="radio" class="custom-control-input" name="gateway">
                                     <div class="payment h-100 card-shadow">
                                        <h4 class="m-0">{{ __('FREE') }}</h4>
                                     </div>
                                  </label>
                               </div>
                        </div>
                     ...
                     <div class="col-lg-4 mt-4 mt-lg-0">
                        <div class="card p-4 card-shadow radius-8 border-0">
                           ...
                           <div class="card-footer pt-0 border-0 bg-white">
                              <div class="d-flex justify-content-between font-weight-bold">
                                ...
                                @endif
                              <div class="mt-2">
                                 <button type="submit" class="btn btn_sm_primary bg-blue btn-block c-white radius-5">{{ __('Generate Payment') }}</button>
                              </div>
                           </div>
                        </div>
                     </div>
